...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elixirhealth/document/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elixirhealth/document/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/document
...
docker-image:
	...
	@docker build --rm=false -t gcr.io/elixir-core-prod/document:snapshot deploy
...
proto:
	@echo "--> Running protoc"
	@protoc pkg/documentapi/document.proto -I. -I vendor/ --go_out=plugins=grpc:.
