...
import (
	...
	"github.com/elixirhealth/document/pkg/documentapi"
...
func getClients() ([]documentapi.DocumentClient, error) {
	addrs, err := parse.Addrs(viper.GetStringSlice(cmd.AddressesFlag))
	if err != nil {
		return nil, err
	}
	dialer := server.NewInsecureDialer()
	clients := make([]documentapi.DocumentClient, len(addrs))
	for i, addr := range addrs {
		conn, err2 := dialer.Dial(addr.String())
		if err != nil {
			return nil, err2
		}
		clients[i] = documentapi.NewDocumentClient(conn)
	}
	return clients, nil
}
